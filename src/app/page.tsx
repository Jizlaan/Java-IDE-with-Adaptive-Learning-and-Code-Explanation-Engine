...
declare global {
  interface Window {
    electronAPI?: {
      ...
      onTriggerRun: (callback: () => void) => void;
...
const Editor = dynamic(() => import("@monaco-editor/react"), { 
  ssr: false, 
  loading: () => <div className="h-full w-full flex items-center justify-center bg-[#1A1A1A] text-[#888888] font-mono text-[13px]">Allocating workspace memory...</div> 
});
import { Play, Loader2, Brain, TerminalSquare, ChevronRight, PlayCircle, X, Trophy } from "lucide-react";
import { Panel, Group as PanelGroup, Separator as PanelResizeHandle } from "react-resizable-panels";
import ReactMarkdown from "react-markdown";
import StudentDashboard, { recordSession } from "../components/StudentDashboard";
...
export default function Home() {
  ...
  const [studentName, setStudentName] = useState<string | null>("loading");

  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedName = localStorage.getItem("javaMentorStudentName");
  ...
  const handleSaveName = (name: string) => {
    if (typeof window !== "undefined") {
      localStorage.setItem("javaMentorStudentName", name);
    }
    setStudentName(name);
  };

  const actionsRef = useRef({ run: async () => {}, save: async () => {} });

  useEffect(() => {
    // Keep our ref updated with closures to latest exact state
    actionsRef.current.save = async () => {
      if (typeof window !== "undefined" && window.electronAPI) {
        await window.electronAPI.saveFile(code);
      }
    };
  }, [code]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.electronAPI) {
      window.electronAPI.onFileOpened((data) => {
        ...
        setCurrentFileName(fileName || "Unknown.java");
      });

      window.electronAPI.onTriggerRun(() => {
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      setOutput(`Failed to run code: ${errorMsg}`);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  useEffect(() => {
    actionsRef.current.run = handleRunCode;
  }, [handleRunCode]);
  ...
  const fetchChatResponse = async (newMessages: {role: string, content: string, hidden?: boolean}[], codeContext?: string, errorContext?: string) => {
    ...
    try {
      ...
      if (codeContext && errorContext) {
        // Prevent huge payloads to the backend API which can cause 413 Payload Too Large
        payload.code = codeContext.length > 3000 ? codeContext.substring(0, 3000) : codeContext;
        payload.error_stack = errorContext.length > 3000 ? errorContext.substring(errorContext.length - 3000) : errorContext;
      }
      ...
      if (data.success) {
        ...
        // If this chat was triggered by a compiler/runtime error, record it!
        if (errorContext) {
          const shortError = errorContext.trim().split('\n')[0].substring(0, 100);
          recordSession(shortError, data.detectedSmells || []);
        }
      } else {
  ...
  const handleEditorBeforeMount = (monaco: import('@monaco-editor/react').Monaco) => {
  ...
  // Memoize markdown components to prevent catastrophic lag during typing!
  const markdownComponents = useMemo(() => ({
    h1: ({node: _, ...props}: any) => <h1 className="text-base font-bold my-2 text-white" {...props} />,
    h2: ({node: _, ...props}: any) => <h2 className="text-[14px] font-bold mt-4 mb-2 text-[#E0E0E0]" {...props} />,
    h3: ({node: _, ...props}: any) => <h3 className="text-[13px] font-bold mt-3 mb-1 text-[#c678dd]" {...props} />,
    p: ({node: _, ...props}: any) => <div className="mb-2 last:mb-0 text-[#cccccc]" {...props} />,
    ul: ({node: _, ...props}: any) => <ul className="list-disc pl-4 mb-3 space-y-1" {...props} />,
    ol: ({node: _, ...props}: any) => <ol className="list-decimal pl-4 mb-3 space-y-1" {...props} />,
    li: ({node: _, ...props}: any) => <li className="mb-0 text-[#cccccc]" {...props} />,
    a: ({node: _, ...props}: any) => <a className="text-[#599e5e] hover:text-[#7CB165] hover:underline font-medium" target="_blank" rel="noopener noreferrer" {...props} />,
    strong: ({node: _, ...props}: any) => <strong className="font-semibold text-[#eeeeee]" {...props} />,
    pre: ({node: _, ...props}: any) => <pre className="bg-[#000] p-2.5 rounded-md text-[11.5px] font-mono border border-[#333] overflow-x-auto shadow-inner mb-3 mt-1 text-[#D4D4D4]" {...props} />,
    code: ({node: _, className, children, ...props}: React.HTMLAttributes<HTMLElement> & { node?: unknown }) => {
      const isBlock = className?.includes('language-') || String(children).includes('\n');
      return isBlock 
        ? <code className={className} {...props}>{children}</code> 
        : <code className="bg-[#000] text-[#D4D4D4] px-1 py-0.5 rounded text-[11.5px] font-mono border border-[#333]" {...props}>{children}</code>;
    }
  }), []);
  ...
  return (
    <div className="flex flex-col h-screen text-[#E0E0E0] font-sans overflow-hidden bg-[#0D0D0D]">
      ...
      <header className="flex items-center justify-between px-4 h-11 shrink-0 bg-[#121212] border-b border-[#333333] shadow-sm z-10">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center text-[13px] font-medium text-[#A0A0A0]">
            ...
            src
            <ChevronRight className="w-4 h-4 mx-1 text-[#555555]" />
            <span className="text-white drop-shadow-sm">{currentFileName}</span>
          </div>
        </div>
        ...
        {/* Right: Layout Spacer / Dashboard */}
        <div className="flex items-center space-x-4 w-[36px]">
          <button 
            onClick={() => setIsDashboardOpen(true)}
            className="p-1.5 hover:bg-[#2A2A2A] rounded-md transition-colors group cursor-pointer"
            title="Student Dashboard & Badges"
          >
            <Trophy className="w-4 h-4 text-[#888888] group-hover:text-[#F1C40F]" />
          </button>
        </div>
      </header>
      ...
      <StudentDashboard 
        ...
        studentName={studentName === "loading" || studentName === null ? "Student" : studentName} 
      />
      ...
      {studentName === null && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[2000] p-4 backdrop-blur-sm">
          <div className="bg-[#161616] border border-[#333333] p-7 rounded-lg shadow-2xl max-w-sm w-full animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-[#E0E0E0] text-xl font-semibold mb-2 flex items-center gap-2">
              <Trophy className="w-5 h-5 text-[#F1C40F]" />
              Welcome to Java IDE
            </h2>
            <p className="text-[#A0A0A0] text-sm mb-4 leading-relaxed">
              Please enter your name to begin. This connects your profile to your dashboard streaks and badges. <br/><br/>
              <span className="text-[#F1C40F] opacity-90"><b className="font-semibold">Note:</b> This can only be set once per device.</span>
            </p>
            <input 
              type="text" 
              className="w-full bg-[#0D0D0D] border border-[#333333] rounded-md px-3 py-2 text-[#E0E0E0] outline-none focus:border-[#4DAAFB] focus:ring-1 focus:ring-[#4DAAFB] transition-all mb-4 mt-1"
              placeholder="Your Name (e.g. John Doe)"
              onKeyDown={(e) => {
                if (e.key === 'Enter' && e.currentTarget.value.trim().length > 0) {
                  handleSaveName(e.currentTarget.value.trim());
                }
              }}
              autoFocus
            />
            <div className="flex justify-end mt-2">
               <button 
                 ...
                 className="bg-[#4DAAFB] hover:bg-[#3B8BD4] text-white px-5 py-2 rounded-md text-sm font-medium transition-colors disabled:opacity-50"
               >
                 Get Started
               </button>
            </div>
